            ...
            <table class="table table-bordered text-center datatable table-striped my-only-div-shadow">
              ...
              <tbody class="table-font-size">
                ...
                $query = "SELECT *,p.id AS pur_id,pi.id FROM purchase AS p 
                   ...
                   LEFT JOIN purchase_items AS pi ON pi.purchase_id = p.id where pi.product_code ='old' GROUP BY p.id
                   ORDER BY p.id DESC";
  ...
  function deleteData(purchase_id) {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        window.location.href = "old_details.php?purchaseId=" + purchase_id;
      }
    });
  }
</script>
        ...
        echo "<!DOCTYPE html>
          <html>
            <body>
              ...
              if (result.isConfirmed) {
              window.location.href = 'old_details.php'
              }
              });
              </script>
            </body>
